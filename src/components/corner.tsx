...
export function Corner({ size, padding = 20, inverted, children, className, twoCorners }: CornerProps) {
  return (
    <>
      <div className={cn("from-blue-500 to-pink-400 rounded-tl-full", inverted ? "rounded-l-full bg-gradient-to-l" : "rounded-r-full bg-gradient-to-r")} style={{ width: size, height: padding, translate: `${inverted ? `0 ${size}px` : ""}` }} />
        ...
        <div className="relative w-full">
          ...
          {twoCorners ? (
            <>
              ...
              <div className={cn("from-blue-500 to-pink-400 rounded-t-full bg-gradient-to-t absolute left-full rounded-br-full")} style={{ height: size, width: padding, top: `calc(100% - ${size - padding}px)` }} />
            </>
          ) : null}
        </div>
      </div>
